...
private:
	...
	PageDescriptor *split_block(PageDescriptor **block_pointer, int source_order)
	{
		...
		PageDescriptor *previous = *block_pointer;
		PageDescriptor *buddy  = *block_pointer + pages_per_block(source_order - 1);
		remove_block(previous, source_order);
		insert_block(previous, source_order - 1);
	...
	PageDescriptor **merge_block(PageDescriptor **block_pointer, int source_order)
	{
		...
		if (source_order + 1 == MAX_ORDER)
			return block_pointer;
		PageDescriptor *previous = *block_pointer;
		PageDescriptor *buddy = buddy_of(previous, source_order);
		remove_block(previous, source_order);
		remove_block(buddy, source_order);
		if (is_correct_alignment_for_order(previous, source_order + 1))
			return insert_block(previous, source_order + 1);
...
public:
	...
	PageDescriptor *alloc_pages(int order) override
	{
		...
		while(order_copy < MAX_ORDER && !(_free_areas[order_copy]))
			order_copy++;
		PageDescriptor *block_pointer = _free_areas[order_copy];
		while(order_copy > order)
			block_pointer = split_block(&block_pointer, order_copy--);
	...
	void free_pages(PageDescriptor *pgd, int order) override
	{
		...
		PageDescriptor **freed_block = insert_block(pgd, order);
		PageDescriptor *buddy = buddy_of(*freed_block, order);
		PageDescriptor *block_pointer = _free_areas[order];
		int order_copy =  order;
		while(order_copy < MAX_ORDER && block_pointer) {
			if (buddy == block_pointer) {
				freed_block = merge_block(freed_block, order_copy++);
				buddy = buddy_of(*freed_block, order_copy);
				block_pointer = _free_areas[order_copy];
			}
			else if (buddy != NULL)
				break;
			else
	...
	{
		int order_copy = 0;
		PageDescriptor *temp_pdg;
		bool isreserved = false;
		while (!isreserved && order_copy < MAX_ORDER) {
			PageDescriptor *block_pointer = _free_areas[order_copy];
			while(block_pointer){
				if ( pgd < (block_pointer + pages_per_block(order_copy)) && pgd >= block_pointer) {
					temp_pdg = block_pointer;
					while(order_copy > 0) {
						temp_pdg = split_block(&temp_pdg, order_copy--);
						PageDescriptor *buddy = buddy_of(temp_pdg, order_copy);
						if ( (buddy + pages_per_block(order_copy)) > pgd && pgd >= buddy )
							temp_pdg = buddy;
					}
					isreserved = true;
					break;
	...
	bool init(PageDescriptor *page_descriptors, uint64_t nr_page_descriptors) override
	{
		...
		int total_blocks = nr_page_descriptors / pages_per_block(MAX_ORDER-1);
		PageDescriptor *pgd = page_descriptors;
		_free_areas[MAX_ORDER-1] = pgd;
		for(int i=0; i < total_blocks; i++) {
			pgd->next_free = pgd + pages_per_block(MAX_ORDER-1);
			pgd = pgd->next_free;
		}
		pgd->next_free = NULL;
		return true;
